...
Last session: Session 28 — Apify for IG/TikTok (async poll + lazy cascade link→comments→transcript).
Next action: Set `APIFY_TOKEN` (+ keep `SUPADATA_API_KEY`) in `.env` + Vercel Preview, then manual end-to-end on real IG/TikTok URLs: confirm comment-sourced recipes (transcript skipped when comments suffice — check logs), blog-link reels parse via extractFromWebsite, and the poll completes under the 60s cap. Apify actor input/output field mappings (IG `latestComments`/`likesCount`, TikTok comments-scraper `maxItems`/`diggCount`) are from docs — verify against real run output and adjust if needed. Daily rotation under 60s still NOT proven under load (likely a shared-DB cache hit) — watch the first-of-day cold generation. DEFERRED: SSE progress; YouTube creator-metadata enrichment. Plan file: .claude/plans/review-import-spec-sharded-lamport.md.
...
- **Actors:** IG = `apify~instagram-scraper` (1 run → `caption`/`latestComments`/`ownerUsername`). TikTok = `clockworks~tiktok-comments-scraper` (comments only — `text`/`diggCount`/`uniqueId`; **no caption**, creator derived from URL `@handle`). Apify output is untyped + actor-specific → `parseSocial` is defensive; **field mappings are from docs and need real-run verification** (esp. TikTok input limit field + IG `likesCount`).
- TikTok caption isn't scraped (short; transcript+comments cover it) — a caption actor is a future add. The old TikTok→Supadata path was removed (now Apify).
