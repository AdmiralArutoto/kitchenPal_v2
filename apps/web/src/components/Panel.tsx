...
export default function Panel({
  ...
  return (
    <div
      className={`rounded-[10px] ${bordered ? 'border border-border-subtle' : ''} bg-bg-card shadow-[0px_10px_7.5px_-1px_rgba(0,0,0,0.1),0px_4px_3px_-1px_rgba(0,0,0,0.1)] ${paddingClasses[padding]} ${className}`}
      {...rest}
    >
      {children}
    </div>
